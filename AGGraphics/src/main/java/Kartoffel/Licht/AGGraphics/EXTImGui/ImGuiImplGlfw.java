...
import Kartoffel.Licht.AGGraphics.EXTImGui.ImGuiIntegration.bref;
import imgui.ImGui;
...
public class ImGuiImplGlfw {
    ...
    /**
     * {@link Integer} viewport UID : {@link AGVGraphics} graphics
     */
    HashMap<Integer, Window> windows = new HashMap<Integer, Window>();
    HashMap<Integer, AGDrawer> drawers = new HashMap<Integer, AGDrawer>();
    HashMap<Integer, bref> brefs = new HashMap<Integer, bref>();
    Window mainWindow;
    AGVGraphics graphics;
    ImGuiIntegration igi;
    public ImGuiImplGlfw(ImGuiIntegration t) {
		this.igi = t;
	}

    // We gather version tests as define in order to easily see which features are version-dependent.
    protected static final int glfwVersionCombined = GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 + GLFW_VERSION_REVISION; //Assuming we use GLFW 3.4
...
//        }
    ...
    public void keyCallback(final long window, final int keycode, final int scancode, final int action, final int mods) {
        ...
        final ImGuiIO io = ImGui.getIO();
        final int imguiKey = KeyMappings.getImKey(key);
    ...
    public boolean init(final Window window, final boolean installCallbacks) {
        final ImGuiIO io = ImGui.getIO();

        io.setBackendPlatformName("imgui-java_impl_glfw#AGGraphics");
...
//        }
        ...
        this.graphics = window.getParent();

        data = newData();
        data.window = window.getGlfwHandle();
        data.time = 0.0;
        data.wantUpdateMonitors = true;
...
//        }
        ...
        // Chain GLFW callbacks: our callbacks will call the user's previously installed callbacks, if any.
        if (installCallbacks) {
            installCallbacks(window.getGlfwHandle());
        }
        ...
        mainViewport.setPlatformHandle(window.getGlfwHandle());
        if (IS_WINDOWS) {
            mainViewport.setPlatformHandleRaw(GLFWNativeWin32.glfwGetWin32Window(window.getGlfwHandle()));
        }
        if (IS_APPLE) {
            mainViewport.setPlatformHandleRaw(GLFWNativeCocoa.glfwGetCocoaWindow(window.getGlfwHandle()));
        }
        if (io.hasConfigFlags(ImGuiConfigFlags.ViewportsEnable)) {
...
//        if (glfwHasGamepadApi) {
            try (GLFWGamepadState gamepad = GLFWGamepadState.create()) {
                ...
                mapButton = (keyNo, buttonNo, _) -> io.addKeyEvent(keyNo, gamepad.buttons(buttonNo) != 0);
                mapAnalog = (keyNo, axisNo, _, v0, v1) -> {
...
//    		glfwWindowHint(GLFW.GLFW_MOUSE_PASSTHROUGH, GLFW_TRUE);
...
//            }
            ...
            vd.window = glfwCreateWindow((int) vp.getSizeX(), (int) vp.getSizeY(), "No Title Yet", NULL, 0);
            vd.windowOwned = true;
            vp.setPlatformHandle(vd.window);
            
            
            var img = new GLFWImage.Buffer(MemoryUtil.memCalloc(GLFWImage.SIZEOF*1));
            var data = MemoryUtil.memCalloc(16*16*4);
            for(int a = 0; a < 16*16*4; a+=4) {
            	data.put(a+0, (byte) 0);
            	data.put(a+1, (byte) 0);
            	data.put(a+2, (byte) 0);
            	data.put(a+3, (byte) 3);
            }
            img.get(0).set(16, 16, data);
            GLFW.glfwSetWindowIcon(vd.window, img);
...
//    private static final class SwapBuffersFunction extends ImPlatformFuncViewport { //In Vulkan, buffers get directly swapped after rendering (in this implementation of the backend).
//        @Override
//        public void accept(final ImGuiViewport vp) {
//            final ViewportData vd = (ViewportData) vp.getPlatformUserData();
//            if (vd != null) {
...
//    }
    ...
    protected void initPlatformInterface() {
        ...
        platformIO.setRendererCreateWindow(new ImPlatformFuncViewport() {
			
			@Override
			public void accept(ImGuiViewport vp) {
				var window = graphics.createWindow(vp.getPlatformHandle(), true, mainWindow.getCanvas().getRenderpass());
				windows.put(vp.getID(), window);
				drawers.put(vp.getID(), graphics.createDrawer());
				brefs.put(vp.getID(), igi.new bref());
			}
		});
        platformIO.setRendererDestroyWindow(new ImPlatformFuncViewport() {
			...
			public void accept(ImGuiViewport vp) {
				...
				windows.remove(vp.getID());
			}
		});
        platformIO.setRendererSetWindowPos(new ImPlatformFuncViewportImVec2() {
			
			@Override
			public void accept(ImGuiViewport vp, ImVec2 imVec2) {
				if(!drawers.containsKey(vp.getID())) return;
				drawers.get(vp.getID()).join();
				windows.get(vp.getID()).resizeDisplayBuffers((int)imVec2.x, (int)imVec2.y);
				brefs.get(vp.getID()).hash = 0;
			}
		});
        platformIO.setRendererRenderWindow(new ImPlatformFuncViewport() {
			...
			public void accept(ImGuiViewport vp) {
				...
				if(index > -1) {
					drawer.startDraw();
					drawer.beginRenderpass(window.getCanvas(), index, 0, 0, window.getWidth(), window.getHeight());
					igi.updateBuffers(vp.getDrawData(), bref);
					if(bref.indices != null) {
						igi.draw(drawer, vp.getDrawData(), bref);
					}
					drawer.endRenderpass();
					drawer.endDraw();
					window.getCanvas().submitDisplay(drawer, window.getParent().getMainQueue());
				}
			}
		});
